...
Inter-Region Peering | リージョン間ピアリング | 別々のリージョンに位置する2つのVPCを接続することをリージョン間ピアリングといいます。これは別のリージョンに位置する2つのVPCを接続します。
Interface | インターフェイス | インスタンスをネットワークに接続するための装置をネットワークインターフェイスと呼びます。
Service Gateway | サービスゲートウェイ | 選択可能なサービスをインターネットを経由せずに安全に接続します。 VPCに割り当てられたIPと選択されたサービスが1:1で接続されます。
