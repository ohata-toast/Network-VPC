...
リージョン間 ピアリングは、別々のリージョンに作成された2つのVPCを接続する機能です。同じリージョンのVPCはピアリングを利用して接続することができますが、リージョンが異なるVPCを接続することはできません。したがってリージョンが異なる2つのVPCを接続する機能を提供しますが、これをリージョン間 ピアリングといいます。

> [参考]リージョン間 ピアリングは、別々のリージョンの2つのVPCを接続します。別のVPCを経由して別のVPCに 接続することはサポートしません。 A <-> B <-> C接続でAとCは接続されません。
* リージョン間 ピアリングは韓国(坪村)リージョンと韓国(パンギョ)リージョンの一部VPCでのみ利用できます。
* 同じアカウント、同じプロジェクトの2つのVPCのみ接続できます。
* リージョン間 ピアリングを作成すると、接続された別のリージョンで自動的に作成されます。
* リージョン間 ピアリングを削除すると、接続された別のリージョンから自動的に削除されます。
* 2つのVPCのIPアドレス領域は重複して使用できません。
* ピアリングされた両方のVPCのルーティングテーブルに別々のルートを設定すると通信が可能です。
    * ルートの対象CIDRに相手VPCのIPアドレス領域を入力し、ゲートウェイリストにリージョン間 ピアリングの名前を持つINTER_REGION_PEERING項目を選択してルートを追加します。
    * ルートを追加したルーティングテーブルに接続されたサブネットでのみ通信が可能です。
    * 「基本ルーティングテーブル」ではなくルーティングテーブルにルートを追加すると、ルーティングテーブルに接続されたサブネットでピアリング通信が可能です。
    * ピアリング作成時、サブネットがないVPCを指定すると、ピアリングは作成に失敗します。
...
サービスゲートウェイはFloating IPを使用せず、トラフィックがインターネットを経由しなくてもVPC外部のサービスを利用できりうようにします。サービスゲートウェイ作成時に選択されたサービスと、自動的に割り当てられたIPは1:1接続関係を維持し、VPCではサービスゲートウェイIPを利用して内部ネットワークを経由して対象サービスを安全に利用できます。 

* サービスゲートウェイ作成時、サービスリストに提供されるサービスのみ利用可能です。
* 1つのサービスゲートウェイは1つのサービスと1:1で接続されます。
* サービスゲートウェイIPで接続すると、サービスゲートウェイ作成時に選択したサービスに接続され、サービスを利用できます。
    * 例) Object Storage使用
        * wget http://{SERVICE_GW_IP}/v1/
* サービスゲートウェイは、サービスゲートウェイが作成されたVPC内でのみ使用できます。
* URLアクセスをサポートしません。URL アクセスが必要な場合は、次の例のように/etc/hostsファイルにURLを追加する必要があります。
    * 例) /etc/hostsファイル
