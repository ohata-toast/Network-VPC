...
* 한국(판교), 한국(평촌) 리전에서는 프라이빗 IP DNS 기능의 사용 / 사용안함을 지정할 수 있습니다.
  * 프라이빗 IP DNS 기능 사용 설정 시 VPC의 인스턴스들을 재시작해야 합니다.
  * Private DNS 서비스를 사용하지 않더라도 프라이빗 IP DNS 사용 설정 시 도메인을 사용할 수 있습니다.
  * 프라이빗 IP DNS 기능 사용 여부와 상관없이 네트워크 인터페이스에 프라이빗 IP DNS 항목이 노출됩니다.
