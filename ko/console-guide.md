...
* 한쪽 리전에서 생성시  연결된 리전에 리전간 피어링이 자동으로 생성됩니다.
* 한쪽 리전에서 삭제시 연결된 리전에서 리전간 피어링이 자동으로 삭제 됩니다.
* 두 VPC의 IP 주소 영역이 겹치는 것을 허용합니다. 
    * 이런 경우 subnet을 겹치지 않게 설정하고 라우트를 설정해야합니다.
* 피어링한 양쪽 VPC의 라우팅 테이블에 별도의 라우트를 설정해야 통신이 가능합니다.
