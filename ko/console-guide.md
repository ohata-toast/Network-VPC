...
* 한국(판교), 한국(평촌) 리전에서 프라이빗 IP DNS 기능의 사용 / 사용안함을 지정할 수 있습니다. 동일 VPC내의 통신에서만 사용할 수 있습니다.
  * 프라이빗 IP DNS는 네트워크 인터페이스에 IP 기반으로 할당되는 DNS 호스트 이름입니다. 동일 VPC내의 통신에서만 사용할 수 있습니다.
  * Private DNS 서비스를 사용하지 않고 프라이빗 IP DNS 기능 사용 설정 시 VPC의 인스턴스들을 재시작해야 합니다.
