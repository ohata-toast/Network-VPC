...
## 서비스 게이트웨이
서비스 게이트웨이는 VPC외부의 특정 서버에 접근을 지원합니다. 서비스 게이트웨이 생성시 선택된 특정 서버와 1:1 관계를 유지하며 VPC내부에서 서버와 proxy 역할을 수행합니다.

* 한 개의 서비스 게이트웨이는 한 개의 서비스 엔트포인트와 1:1로 연결됩니다.
* VPC내부에 속한 서비스 게이트웨이 인터페이스 IP로 접속시 서비스 게이트웨이가 연결하는 외부 서버로 연결됩니다.
