...
서비스 게이트웨이는 플로팅IP를 쓰지 않고 트래픽이 인터넷을 경유하지 않고도 VPC 외부의 서비스를 이용할 수 있게 합니다. 서비스 게이트웨이 생성 시 선택된 서비스와 자동으로 추가된 사설IP는 1:1 연결 관계를 유지하며, VPC는 사설IP를 이용하여 내부 네트워크를 통해 대상 서비스를 안전하게 이용할 수 있습니다. 

* 서비스 게이트웨이 생성 시 서비스 목록에 제공되는 서비스만 이용이 가능합니다.
* 한 개의 서비스 게이트웨이는 한 개의 서비스와 1:1로 연결됩니다.
* 서비스 게이트웨이에 의해 생성된 VPC 사설IP로 접속 시 서비스 게이트웨이가 연결하는 서비스로 연결됩니다.
