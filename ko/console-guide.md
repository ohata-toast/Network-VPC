...
### 공유 기능
VPC 공유 기능을 사용하면 다른 프로젝트에서 내 프로젝트의 VPC에 접근하는 것을 허용합니다. 

* VPC를 공유 받은 프로젝트에서는 해당 VPC애 접근하여 인스턴스, 네트워크 인터페이스와 같은 자원을 직접 연결할 수 있습니다. 

> [참고] 
> 공유 받은 VPC에 대한 수정, 삭제는 허용하지 않습니다.

이러한 구성은 서로 다른 프로젝트의 리소스를 동일 VPC 내에서 통신하도록 해야할 때 유용합니다.
예를 들어 웹서비스를 관리하는 부서의 프로젝트에서 웹서버 인스턴스를 운영하고, 보안 정책을 관리하는 부서의 프로젝트에서 방화벽 인스턴스를 운영하는 구성이 필요할 때, 웹서비스 관리 부서 프로젝트의 VPC를 보안 정책 관리부서의 프로젝트에 공유함으로써 방화벽 인스턴스를 웹서버 인스턴스와 같은 VPC에 연결하도록 구성할 수 있습니다.

#### VPC 공유
VPC를 공유하기 위해서는 공유할 VPC를 선택한 후 **다른 프로젝트에 공유** 버튼을 누른 다음, 공유할 대상 프로젝트의 테넌트 ID 값을 직접 입력합니다.
VPC를 공유 받은 프로젝트에서는 인스턴스, 인스턴스 템블릿, 네트워크 인터페이스, NKS, NCS 생성 화면의 VPC 선택 목록에서 공유 받은 VPC와 서브넷을 선택할 수 있습니다.

> [참고]
> VPC 메뉴의 목록 화면에는 내 프로젝트의 VPC만 노출되며, 공유 받은 VPC는 노출되지 않습니다.

#### VPC 공유 해제
VPC의 공유를 해제하기 위해서는 VPC를 선택한 후 **다른 프로젝트에 공유** 버튼을 누른 다음, 이전에 공유한 프로젝트의 테넌트 ID 목록에서 대상 프로젝트의 테넌트 ID를 삭제합니다.
단, 공유 해제는 대상 프로젝트에서 해당 VPC에 연결한 모든 리소스를 삭제한 상태에서만 가능합니다.


> [주의]
> 다른 프로젝트에 공유한 VPC는 VPC를 공유 받은 프로젝트에서 해당 VPC에 생성한 자원을 모두 삭제해기 전에는 삭제할 수 없습니다.
> 또한 공유된 VPC가 있는 프로젝트를 삭제하면, 해당 VPC를 공유 받은 다른 프로젝트에서 해당 VPC에 연결하고 있는 네트워크 인터페이스가 예고 없이 삭제될 수 있습니다. 따라서 프로젝트 삭제 전 반드시 자원 정리와 VPC 공유 해제를 수행하시기 바랍니다.
