...
* 한국(판교), 한국(평촌) 리전에서는 프라이빗 IP DNS 기능의 사용 여부를 지정할 수 있습니다.
  * 프라이빗 IP DNS 기능을 사용하면 네트워크 인터페이스에 IP 기반으로 할당되는 DNS 호스트 이름이 자동으로 생성되어 네임서버에 레코드 조회를 할 수 있습니다.
...
Private IP DNS | 프라이빗 IP DNS | 네트워크 인터페이스에 IP 기반으로 할당되는 DNS 호스트 이름입니다. 형식은 다음과 같습니다. <br> ip-{네트워크 인터페이스 IP}.{리전 정보}.nhncloud.internal 
