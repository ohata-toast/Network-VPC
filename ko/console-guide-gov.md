...
### 공유 기능
VPC 공유 기능을 사용하면 다른 프로젝트에서 VPC에 접근하는 것을 허용합니다. 

* VPC를 공유 받은 프로젝트에서는 해당 VPC애 접근하여 인스턴스, 로드 밸런서, 네트워크 인터페이스와 같은 자원을 VPC에 직접 연결할 수 있습니다. 
...
#### VPC 공유
VPC를 공유하기 위해서는 공유할 VPC를 선택한 후 **공유** 버튼을 누른 다음, 공유할 대상 프로젝트의 테넌트 ID 값을 직접 입력합니다.
VPC를 공유받은 프로젝트에서는 인스턴스 생성 화면, 로드 밸런서 생성 화면, 네트워크 인터페이스 생성 화면의 VPC 선택 목록에 공유 받은 VPC와 서브넷이 노출됩니다.
> [참고]
> VPC 메뉴의 목록 화면에는 내 프로젝트의 VPC만 노출되며, 공유받은 VPC는 노출되지 않습니다.

#### VPC 공유 해제
VPC의 공유를 해제하기 위해서는 VPC를 선택한 후 "공유" 버튼을 누른 후, 이전에 공유한 프로젝트의 테넌트 ID 목록에서 대상 프로젝트의 테넌트 ID를 삭제합니다.
